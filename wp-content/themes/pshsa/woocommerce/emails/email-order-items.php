 ...
 * @version     3.0.0
 * Added:       Unit price as either the regular product price or the adjusted product price of the dynamic products discounts extension
 */
...
foreach(WC()->cart->get_cart() as $item => $values):
	if ($values['discounts']['price_adjusted'] != ''):
		$adjusted_products_price_arr[$values['product_id']] = $values['discounts']['price_adjusted'];
	endif;
endforeach;
	...
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
			...
			<td class="td" style="text-align:<?php echo $text_align; ?>; vertical-align:middle; border: 1px solid #eee; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;">
				<?php 
					$product_displayed_price = ($adjusted_products_price_arr[$product->get_id()] != '') ? $adjusted_products_price_arr[$product->get_id()]: $product->get_price();
					echo $product_displayed_price; 
				?>
			</td>
