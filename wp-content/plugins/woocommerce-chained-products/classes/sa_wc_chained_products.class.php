...
class SA_WC_Chained_Products {
	...
	public function __construct() {
		...
		if ( $this->is_wc_gte_30() ) {
	        add_action( 'woocommerce_checkout_create_order_line_item', array( &$this, 'add_chained_products_order_item_meta_3_0' ), 999, 4 );
		} else {
        ...
        //Filter to exclude parent variable product from search result
        add_filter( 'woocommerce_json_search_found_products', array( $this, 'exclude_parent_variable_product_from_search') );	
    }
    ...
    /**
     * Function to exclude variable parent from search result ( for variable products )
     *
     * @param array $products
     * @return array $products   
     */
    public function exclude_parent_variable_product_from_search( $products ) {
     	if ( ! empty ( $products ) ) {
    ...
    public function add_order_item_meta_manually( $item_id, $item )  {
        if ( $this->is_wc_gte_30() ) {
	    	...
	    	$item_to_add = $this->is_wc_gte_30() ? $_product->get_id() : $_product->id;
	        
		    if ( $_product instanceof WC_Product_Variation ) { 
	            $item_to_add = $variation_id = $this->is_wc_gte_30() ? $item_to_add : $_product->variation_id ;
	        }
	            ...
	            foreach ( $chained_product_details as $parent_id => $chained_values ) {
	                ...
	                if ( in_array( $item_to_add, $chained_product_ids) ) { 
			        	...
			        	$args = array(
			                ...
			                '_variation_data'     => isset( $variation_id ) ? $product->get_variation_attributes() : '',					
	...
	public function add_chained_product_meta_to_subscription( $id, $item_id, $product, $qty, $args ) {
		if( isset( $args['chained_product_of'] ) ) {
            wc_add_order_item_meta( $item_id, '_chained_product_of', $args['chained_product_of'] );
  		}
	}
	...
	/**
	 * Add Chained Products order meta in new order ( For WC - v3.0 )
	 *
	 ...
	 * @param WC_Order $order  
	 */
	public function add_chained_products_order_item_meta_3_0( $item, $cart_item_key, $product_values, $order ) {
						...
						<p class="form-field <?php if( ! isset( $product_detail[$product_id] ) ) echo 'nested_chained_products_'.$chained_parent_id; ?>" id="chained_products_row_<?php echo $chained_parent_id . '_' . $row_loop; ?>">
							...
							if ( $this->is_wc_gte_30() ) { ?>
                                <select class="wc-product-search" style="width: 50%;" id="chained_products_ids_<?php echo $chained_parent_id . '_' . $row_loop; ?>" name="<?php if( ! isset( $product_detail[$product_id] ) ) echo 'nested_'; ?>chained_products_ids[<?php echo $chained_parent_id; ?>][<?php echo $row_loop; ?>]" data-placeholder="<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>" 
									data-action="woocommerce_json_search_products_and_variations" data-exclude="<?php echo $total_ids_to_exclude; ?>" <?php if( ! isset( $product_detail[$product_id] ) ) echo 'disabled';?> >
								    <?php 
					...
					<p class="form-field" id="chained_products_row_<?php echo $chained_parent_id . '_' . $row_loop; ?>">
						...
						if ( $this->is_wc_gte_30() ) { ?>
						    <select class="wc-product-search" style="width: 50%;" id="chained_products_ids_<?php echo $chained_parent_id . '_' . $row_loop; ?>" name="chained_products_ids[<?php echo $chained_parent_id; ?>][<?php echo $row_loop; ?>]" data-placeholder="<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>" 
                                data-action="woocommerce_json_search_products_and_variations" data-exclude="<?php echo $total_ids_to_exclude; ?>" /></select> <?php
                        } 
                        else { ?> 
				...
				jQuery('#add_chained_products_row_'+chained_id).off('click').on( 'click', function() { 
					...
					var new_row = " <p class='form-field' id='chained_products_row_"+chained_id+"_"+row_id+"'>\
										...
										<?php if ( $this->is_wc_gte_30() ){?>\
											...
											data-action='woocommerce_json_search_products_and_variations' data-exclude='<?php echo $total_ids_to_exclude; ?>'/></select>\
										<?php } else { ?>\
