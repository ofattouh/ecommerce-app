...
global $ppce_admin_ui_non_administrator;
$ppce_admin_ui_non_administrator = new PPCE_Admin_UI_NonAdministrator();

class PPCE_Admin_UI_NonAdministrator
{
	function __construct() {
		...
		if ( 'nav-menus.php' == $pagenow ) {
			global $ppce_nav_menu_ui;
			require_once( dirname(__FILE__).'/filters-nav-menu-ui_ppce.php' );
			$ppce_nav_menu_ui = new PPCE_NavMenuUI();
		}
		
		add_action( 'admin_print_scripts', array(&$this, 'header_scripts') );
		add_action( 'admin_print_footer_scripts', array(&$this, 'footer_scripts') );
		
		add_filter( 'pp_posts_request_bypass', array( &$this, 'flt_posts_request_bypass' ), 10, 3 );
	...
	function header_scripts() {
		...
		if ( empty($current_user->allcaps['upload_files']) && ! empty($current_user->allcaps['edit_files']) ) : ?>
			<style type="text/css">
			#adminmenu ul.wp-submenu li a[href="media-new.php"]{display:none;} 
			</style>
		<?php endif;
		...
		if ( empty( $current_user->allcaps['edit_theme_options'] ) && ( 'nav-menus.php' == $pagenow ) ) : ?>
			<style type="text/css">
			#nav-menu-header .delete-action a{display:none;}
			</style>
		<?php endif;
	}
...
/* ]]> */
		...
		<?php endif;  // limited nav menu manager?
	...
	function filter_add_new_button() {
		...
		if ( in_array( $pagenow, array( 'edit.php', 'post.php', 'post-new.php', 'upload.php' ) ) ) {
			$_post_type = ( ! empty($_REQUEST['post_type']) ) ? pp_sanitize_key($_REQUEST['post_type']) : 'post';

			if ( ! in_array( $_post_type, pp_get_enabled_post_types() ) )
				return;
