...
class PPCE_BulkEdit {
	public static function add_author_pages( $data ) {
		...
		$post_type = ( isset( $data['member_page_type'] ) ) ? pp_sanitize_key($data['member_page_type']) : '';
		...
		if ( empty( $data['users'] ) ) {
			...
			$meta_keys = array( '_wp_page_template', '_thumbnail_id' );
			$post_meta = array();
			$terms = array();
			...
			if ( $pattern_id ) {
				if ( ! is_numeric($pattern_id) ) {
					$slug = pp_sanitize_key($pattern_id);
					$pattern_post = $wpdb->get_row( $wpdb->prepare( "SELECT ID, post_content, post_parent FROM $wpdb->posts WHERE post_name = %s AND post_type = %s LIMIT 1", $slug, $post_type ) );
				} else {
			...
			if ( ! empty($pattern_post) && ( $post_type == $pattern_post->post_type ) && current_user_can( 'edit_post', $pattern_post->ID ) ) {
				...
				foreach( $taxonomies as $taxonomy ) {
					...
					$terms[$taxonomy] = ppce_get_object_terms( $pattern_post->ID, $taxonomy, array( 'fields' => 'ids', 'pp_no_filter' => true ) );
				}
			} else {
				$post_content = '';
				$post_parent = 0;
			...
			$users_done = $wpdb->get_col( $wpdb->prepare( "SELECT p.post_author FROM $wpdb->postmeta AS pm INNER JOIN $wpdb->posts AS p ON pm.post_id = p.ID WHERE pm.meta_key = '_pp_auto_inserted' AND pm.meta_value = %s", "{$pattern_post_id}:{$post_parent}" ) );
			...
			if ( ! count($data['users'] ) ) {
				...
				foreach( $data['users'] as $user_id ) {
					...
					if ( current_user_can( 'edit_user', $user_id ) ) {
						...
						$post_status = ( pp_get_option('publish_author_pages') ) ? 'publish' : 'draft';
