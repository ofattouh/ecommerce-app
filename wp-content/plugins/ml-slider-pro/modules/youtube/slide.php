...
class MetaYouTubeSlide extends MetaSlide {
    ...
    public function get_admin_tabs() {
        ...
        $general_tab = "<ul>
                            <li><label><input type='checkbox' name='attachment[{$slide_id}][settings][showInfo]' {$showInfo_checked}/>" . __('Show the title on the video', 'ml-slider-pro') ."</label></li>
                            <li><label><input type='checkbox' name='attachment[{$slide_id}][settings][showControls]' {$showControls_checked}/>" . __('Enable controls in the player', 'ml-slider-pro') ."</label></li>
                            <li><label><input type='checkbox' name='attachment[{$slide_id}][settings][showRelated]' {$showRelated_checked}/>" . __('Show related videos', 'ml-slider-pro') ."</label></li>
                            <li><label><input type='checkbox' name='attachment[{$slide_id}][settings][autoHide]' {$autoHide_checked}/>" . __('Hide video controls after the video begins', 'ml-slider-pro') ."</label></li>
                            <li><label><input type='checkbox' name='attachment[{$slide_id}][settings][autoPlay]' {$autoPlay_checked}/>" . __('Auto Play', 'ml-slider-pro') ."</label></li>
    ...
    public function get_video_markup($video_id, $ratio) {
        $attrs = array(
            ...
            'class' => "youtube",
            'data-id' => $video_id,
            'data-autoPlay' => isset($this->slide_settings['autoPlay']) && 'on' == $this->slide_settings['autoPlay'] ? '1' : '0',
            'data-showControls' => !isset($this->slide_settings['showControls']) || 'on' == $this->slide_settings['showControls'] ? '1' : '0',
            'data-showRelated' => isset($this->slide_settings['showRelated']) && 'on' == $this->slide_settings['showRelated'] ? '1' : '0',
            'data-showInfo' => isset($this->slide_settings['showInfo']) && 'on' == $this->slide_settings['showInfo'] ? '1' : '0',
            'data-autoHide' => !isset($this->slide_settings['autoHide']) || 'on' == $this->slide_settings['autoHide'] ? '1' : '0',
    ...
    public function get_flex_slider_parameters( $options, $slider_id ) {
        ...
        $options["start"][]  = "    $('#metaslider_{$slider_id} .youtube').each(function() {
                        var youtube = $(this);
                        var autoplay = false;
                        if (youtube.prop('data-autoPlay') === '1') {
                        ...
                        $(this).tubeplayer({{$this->get_tubeplayer_params()}
                            onPlayerPlaying: function(id) {
                                $('#metaslider_{$slider_id} .flex-active-slide .youtube').prop('data-autoPlay', 0);
    ...
    public function get_responsive_youtube_javascript( $javascript, $slider_id ) {
        ...
        $html .= "\n                $(this).tubeplayer({{$this->get_tubeplayer_params()}";
        $html .= "\n                    onPlayerPlaying: function(id) {";
        $html .= "\n                        $('#metaslider_{$slider_id} .rslides1_on .youtube').prop('data-autoPlay', 0);";
        $html .= "\n                    }";
        $html .= "\n                });";
        $html .= "\n            });";
