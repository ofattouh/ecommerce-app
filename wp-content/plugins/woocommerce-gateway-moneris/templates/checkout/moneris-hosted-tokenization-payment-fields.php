...
<fieldset>
	...
	if ( $tokens ) : ?>
		<p class="form-row form-row-wide">
			...
			<?php foreach( $tokens as $token ) : ?>
				...
				<label style="display:inline;" for="wc-moneris-payment-token-<?php echo esc_attr( $token->get_id() ); ?>"><?php printf( __( '%1$s ending in %2$s (expires %3$s)', 'woocommerce-gateway-moneris' ), $token->get_image_url() ? '<img src="' . esc_url( $token->get_image_url() ) . '" />' : esc_html( $token->get_type_full() ), esc_html( $token->get_last_four() ), esc_html( $token->get_exp_month() . '/' . $token->get_exp_year() ) ); ?></label><br />
			<?php endforeach; ?>
		...
		<div class="clear"></div>
	<?php else : ?>
		<input type="hidden" id="wc-moneris-use-new-payment-method" name="wc-moneris-payment-token" class="js-wc-moneris-payment-token" value="" />
	<?php endif; ?>
	<div class="wc-moneris-new-payment-method-form js-wc-moneris-new-payment-method-form" <?php echo ( $tokens ? 'style="display:none;"' : '' ); ?>>
		<p class="form-row form-row-first">
			...
			<iframe class="input-text js-wc-payment-gateway-account-number" id="wc-moneris-account-number" src="<?php echo $iframe_src; ?>" frameborder="0"></iframe>
